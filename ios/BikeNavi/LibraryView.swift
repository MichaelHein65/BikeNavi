                    ...
                    NavigationLink {
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            HStack(spacing: 15) {
                                ...
                                Spacer()
                                Image(systemName: record.dirty ? "internaldrive" : "checkmark.icloud")
                            }.font(.caption).foregroundStyle(.secondary)
                            Text(Date(timeIntervalSince1970: record.document.createdAt), format: .dateTime.day().month(.wide).year())
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                RouteMap(styleURL: state.mapStyleURL, route: document.route, waypoints: document.waypoints,
                         track: document.track, fitRevision: document.id, colorBySurface: true,
                ...
                if let route = document.route {
                    ElevationChart(coordinates: document.kind == .ride ? document.track.map(\.coordinate) : route.coordinates).frame(height: 140)
                ...
                if document.kind == .ride { Text("Belagsfarben: geplante Route · Orange: gefahrene Strecke").font(.caption).foregroundStyle(.secondary) }
            }.padding(20)
            ...
            .task { exportURL = try? GPX.write(document) }
    }
}
