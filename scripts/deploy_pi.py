...
with tarfile.open(archive, "w:gz") as tar:
    for relative in ["compose.yaml", "server/Dockerfile", "server/.dockerignore", "server/requirements.lock", "server/pyproject.toml"]:
...
ssh("chmod 600 /srv/bikenavi/.env && cd /srv/bikenavi && tar xzf release.tar.gz && docker compose up --build -d")
